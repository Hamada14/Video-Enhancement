...
if __name__ == '__main__':
	...
	frames = extract_frames(SOURCE_VIDEO_PATH, INTERMEDIATE_FRAMES_FOLDER, FRAME_FORMAT)
	print(frames)

	frames_full_path = get_full_frame_path(INTERMEDIATE_FRAMES_FOLDER, FRAME_FORMAT)

	for index in range(frames - 2) :
		print(index)
		first_image_path = (frames_full_path % (2 * index))
		second_image_path = (frames_full_path % (2 * (index + 1)))
		estimated_image_path = (frames_full_path % (2 * index + 1))
		tensorFirst = torch.FloatTensor(numpy.array(PIL.Image.open(first_image_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		tensorSecond = torch.FloatTensor(numpy.array(PIL.Image.open(second_image_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		tensorOutput = estimate(tensorFirst, tensorSecond)

		PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8)).save(estimated_image_path)
	build_video(INTERMEDIATE_FRAMES_FOLDER, FRAME_FORMAT, FPS, 2 * (frames - 2), RESULTANT_VIDEO_PATH, EXTENSION)
# end
