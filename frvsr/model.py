...
class FRVSR():
    def __init__(self, batch_size, frames_len, height, width, channels, flow_depth, check_point_path):
        ...
        lr_frame_input = tf.placeholder(dtype=tf.float32)
        flow_input = tf.placeholder(dtype=tf.float32)
        hr_frame_input = tf.placeholder(dtype=tf.float32)


        def step(previous_output, current_input):

            lr_frame, flow = tf.unstack(current_input)
...
#        reshape_flow = tf.reshape(flow_input, [batch_size, frames_len, 1, height, width, flow_depth])
        ...
        reshaped_flow = tf.reshape(reshaped_flow, [frames_len, batch_size, height, width, channels])

        batch_input = tf.stack([reshaped_lr, reshaped_flow], axis=1)
